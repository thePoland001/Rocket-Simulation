...
tanks_shape = CylindricalTank(radius = 0.075, height = 4.0, spherical_caps = True)
...
oxidizer_tank = MassFlowRateBasedTank(
    ...
    flux_time=5,
    initial_liquid_mass=50,
    initial_gas_mass=0.01,
    liquid_mass_flow_rate_in=0,
    liquid_mass_flow_rate_out=lambda t: 50 / 3 * exp(-0.25 * t),
...
fuel_tank = MassFlowRateBasedTank(
    ...
    geometry=tanks_shape,
    flux_time=5,
    initial_liquid_mass=40,
    initial_gas_mass=0.01,
    liquid_mass_flow_rate_in=0,
    liquid_mass_flow_rate_out=lambda t: 40 / 3 * exp(-0.25 * t),
    gas_mass_flow_rate_in=0,
...
test_flight = Flight(
    rocket=erika, environment=env, rail_length=10, inclination=85, heading=0
)
